...
public class Language {
	...
	public Language(String code, String name, String germanName,
			List<String> abbreviatedDays, boolean rightToLeft,
			boolean inAgenda, boolean hasSpecificFormat) {
		this.code = code;
		this.name = name;
	...
	public Language(Entity entity) {
		...
		if (entity.hasProperty(abbreviatedDaysProperty)) {
			abbreviatedDays = (List<String>) entity
					.getProperty(abbreviatedDaysProperty);
		...
		if (entity.hasProperty(hasSpecificFormatProperty)) {
			hasSpecificFormat = (Boolean) entity
					.getProperty(hasSpecificFormatProperty);
		} else {
	...
	/**
	 * @return a string supplement _xx if the language has a specific format, or the empty string otherwise
	 */
